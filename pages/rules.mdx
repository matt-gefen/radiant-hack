...
When a ***Character*** attacks a ***Creature*** they must do the following to deal damage
- roll below their ***STR*** stat for a ***Melee Attack***
- roll below their ***DEX*** stat for a ***Ranged Attack***

The damage an attack deals is based on the Character’s ***Class Attack Damage***.

Likewise, when a ***Creature*** attacks, the ***Character*** must do the following to avoid taking damage
- roll below their ***STR*** against a Melee Attack
- roll below their  ***DEX*** against a Ranged Attack
...
This rule applies to dual wielding two one-handed weapons as well.
...
### Quick Examples
- A Warrior attacking with a long sword would need to roll below their ***STR*** score to deal 1d8 damage to a ***Close*** target.
- A Thief attacking with a bow would need to roll below their ***DEX*** to deal 1d6 damage to a ***Nearby*** target.
...
Once a spell is cast the spellcaster must test their ***Spell Attribute*** - adding the spell's level to the roll. If they fail then they reduce the number of ***Spell Slots*** corresponding to the spell level just cast by 1. When a memorised spell is cast it is not forgotten.
...
***Characters*** learn through defeating and overcoming obstacles. Killing one ***Monster*** won’t bring a revelation of learning to someone. Surviving a dungeon, completing a quest or simply living to tell the tale are the things that bring perspective and growth.

> For every milestone ***Character*** survives they gain a level.

## Gaining levels

When a ***Character*** levels up, their maximum ***Hit Points*** increase by rolling the ***Hit Die*** for the class. Also a player should roll a d20 for each ***Attribute***, if the result is higher - that ***Attribute*** increases by 1.
...
Magic weapons and armor typically add +1 to any attribute being tested whilst using the weapon and +1 to each damage dice rolled. More powerful weapons (+2/3) can be found if the ***GM*** includes them.
